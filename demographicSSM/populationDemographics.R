...
popnGrowthModelUpdated <- nimbleCode({
  ...
  for(i in 1:M){
    ...
    # Transition Model
    for(t in 2:T){
      N[i,t] ~ T(dpois(N[i, t-1]*gamma[i, t-1]),1, Nst )
      #N[i,t] ~ dpois(N[i, t-1]*gamma[i, t-1] + rho[t-1])
      log(gamma[i, t-1]) <- loggam[i, t-1]
      loggam[i, t-1] <- alpha.gam + beta.gam[1]*elev[i] + beta.gam[2]*pow(elev[i], 2) + beta.gam[3]*forest[i]
    }
...
newModelUpdated <- nimbleModel(popnGrowthModelUpdated,
                               data = data,
                               constants = constants,
                               inits = inits,
                               check = FALSE)
...
example2UpdatedModelTrue <- spartaNimUpdates(model = newModelUpdated , #nimble model
                                             ...
                                             additionalPars = c("N", "gammaX", "popindex"),
                                             n.iter = 1000, #(nIterations - nBurnin)/nThin,
                                             n.chains = nChains,
                                             n.burnin = 100,
                                             ...
                                             propCov1 =  c(0.5, 0.5, 0.5, 1, 1, 1) * diag(6),
                                             propCov =  c(rep(1, 7), 0.5) * diag(8),
                                             pfControl = list(saveAll = TRUE,
                                                              timeIndex = 2,
                                                              smoothing = FALSE,
                                                              #mcmc = TRUE,
